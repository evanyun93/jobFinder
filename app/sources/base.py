...
class SourceConfigError(Exception):
    """API 키/설정이 잘못됐다. 재시도해도 소용없고 사람이 고쳐야 한다.

    일시적 네트워크 오류와 구분하려고 따로 뒀다 — 전자는 스택트레이스가
    필요하지만 후자는 '키를 고치세요' 한 줄이면 된다."""
...
class Job:
    ...
    def haystack(self) -> str:
        ...
        parts = [self.title, self.company, self.region, self.career,
                 ...
                 # 본문까지 봐야 "Python", "SQL" 같은 기술 키워드가 걸린다.
                 # 제목만 보면 "백엔드 개발자" 한 줄이라 대부분 놓친다.
                 self.extra.get("description", "")]
